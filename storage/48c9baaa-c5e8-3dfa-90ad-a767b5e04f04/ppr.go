...
func upload_file(s string) string {
	// """Uploads a file"""
	...
	c,_:=req.Post(endpoint, req.FileUpload{
		...
		FileName:  s, //Filename is the name of the file that you wish to upload. We use this to guess the mimetype as well as pass it onto the server
	},header)
	fmt.Println(c)
	return c.String()
	...
	// 	"Authorization": val,
	// }
	// r, err := req.Post(endpoint, header, f)
	// value,_:=r.ToBytes()
	// c,_:=jsonparser.GetString(value, "BLCODE")
	// if c=="INC23"{
	...
	// req.Post(url, req.File("imroc.png")
	// return
}
