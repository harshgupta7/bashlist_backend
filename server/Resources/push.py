...
class RequestPushBucketURL(APIView):
    ...
    def post(self, request, format='json'):
        ...
        try:
            bucket = Bucket.objects.get(name=bucket_name, shared_with=user, is_shared=True, deleted=False,
                                        available=True)
        ...
        # Case 3: New Bucket
        try:
            bucket = Bucket(owner=user, name=bucket_name, saved_as=secure_filename(bucket_name), size=size,
                            available=False)
...
class PushBucketConfirmation(APIView):
    ...
    def post(self, request, format='json'):
        ...
        s3_bucket_key = '{}__{}'.format(user.s3_bucket_key, bucket.saved_as)
